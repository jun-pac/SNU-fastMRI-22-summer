...
from fastMRI.utils.data import CombinedSliceDataset, SliceDataset
...
class FastMriDataModule(pl.LightningDataModule):
    ...
    def _create_data_loader(
        ...
        if data_partition in ("test") and self.test_path is not None:
            ...
            data_path = self.data_path / f"{data_partition}" / 'kspace'
    ...
    def prepare_data(self):
        ...
        if self.use_dataset_cache_file:
            if self.test_path is not None:
                ...
                test_path = self.data_path / "val" / 'kspace'
            data_paths = [
                self.data_path / "train" / 'kspace',
                self.data_path / "val" / 'kspace',
                test_path,
            ]
            data_transforms = [
