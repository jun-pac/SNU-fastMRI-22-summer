...
class VarNetDataTransform:
    ...
    def __call__(
        ...
        crop_size = (attrs["recon_size"][0], attrs["recon_size"][1])
        image_mask_torch = torch.from_numpy(image_mask)
        
        # Apply augmentations if needed
        if self.use_augment: 
            if self.augmentor.schedule_p() > 0.0:                
                kspace_torch, target_torch = self.augmentor(kspace_torch, target_torch.shape)
                #print(kspace_torch.size())
                #kspace_torch=T.complex_center_crop(kspace_torch,(396,396))
        ...
        if self.mask_func is not None:
            ...
            mask_torch = torch.from_numpy(mask.reshape(*mask_shape).astype(np.float32)).to(torch.bool)
            mask_torch = mask_torch.reshape(*mask_shape)
            
            if not is_pruno:
                kspace_torch = kspace_torch * mask_torch + 0.0
