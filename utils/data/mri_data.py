...
class SliceDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i: int):
        ...
        with h5py.File(fname, "r") as hf:
            if self.is_pruno:
                kspace = hf["kspace_pruno"][dataslice]
            ...
            image_mask = np.asarray(hf["image_mask"][dataslice]) if "image_mask" in hf else None

            attrs = dict(hf.attrs)
            attrs.update(metadata)

        if self.transform is None:
            sample = (kspace, mask, image_mask, target, attrs, fname.name, dataslice)
        else:
            sample = self.transform(kspace, mask, image_mask, target, attrs, fname.name, dataslice,self.is_pruno)
